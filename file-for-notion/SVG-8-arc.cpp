...
//
// 創建SVG文件
std::ofstream svgFile("SVG-8-arc.svg");
//
...
int main() {
    ...
    draw_polyline(border_xy, "lightsteelblue", "sandybrown", 4);
    //
    // 添加 grid h line
    int i_max = 13;
    int x0, x1, y0, y1;
    ...
    //
    draw_polyline(line_1_xy, "none", "magenta", 1);
    draw_polyline(line_2_xy, "none", "magenta", 1);
    //
    make_circle_tuple(400, 300, 150.0, 0, 360);
    circle_tuple_to_string();
    draw_polyline(circle_xy, "none", "red", 1);
    //
    make_arc_tuple(200, 300, 100.0, 0, 180);
    show_arc_tuple_value();
    arc_tuple_to_string();
    draw_polygon(arc_xy, "cyan", "navy", 1);
    // 寫入SVG尾部
