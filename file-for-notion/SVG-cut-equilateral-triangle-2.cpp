...
SVG-cut-equilateral-triangle-2
//
2025-03-05
2026-02-26 
//
...
//
// 創建SVG文件
std::ofstream svgFile("file-2.svg");
//
...
int main() {
    ...
    // svgFile << "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" width=\"650px\" height=\"400px\" viewbox=\"0,150,650,400\" preserveAspectRatio=\"xMinYMin meet\">" 
    //
    svgFile << "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" width=\"650px\" height=\"400px\">" 
            << std::endl;
    //
    // 添加 border polyline
    border_xy = "0,150 650,150 650,400 0,400";
    draw_polygon(border_xy, "lightsteelblue", "red", 3);
    //
    dy = 25.0f;
    // r*sin(i*Pi/3)=25.0
    r = 25/sin(Pi/3);
    ...
    //
    // 添加 grid v line A group
    //
    int i_max = 23;
    y0 = 200;
    y1 = 400;
    for (int i = 1; i < i_max; i=i+3) {
        x0 = i*r;
        x1 = i*r;
        line_v_xy = to_string(x0) + ',' + 
                    ...
                    to_string(x1) + ',' + 
                    to_string(y1);
        draw_polyline(line_v_xy, "none", "blue", 1);
    }
    //
    // 添加 grid v line B group
    //
    i_max = 22;
    y0 = 200;
    y1 = 400;
    for (int i = 3; i < i_max; i=i+3) {
        x0 = i*r-r/2.0;
        x1 = i*r-r/2.0;
        line_v_xy = to_string(x0) + ',' + 
                    to_string(y0) + ' ' + 
                    to_string(x1) + ',' + 
                    to_string(y1);
        draw_polyline(line_v_xy, "none", "red", 1);
    }
    //
    // 添加SVG圖形 for horizontal position group A
    k = round(r);
    for (int i = 1, increment = 3; i < 23; i += increment) {
        text_x = to_string(i*r-15);
        text_y = "190";
        text = to_string(k);
        draw_text(text_x, text_y, "Verdana", "14", "black", text);
        k = k + 3*r;
    } // end for
    //
    // 添加SVG圖形 for horizontal position group B
    k = round(2.5*r);
    for (int i = 3, increment = 3; i < 22; i += increment) {
        text_x = to_string(i*r-0.5*r-15);
        text_y = "190";
        text = to_string(k);
        draw_text(text_x, text_y, "Verdana", "14", "black", text);
        k = k + 3*r;
    ...
    draw_text(text_x, text_y, "Verdana", "14", "black", text);
    // =======================================================
    make_hexagon_tuple(10*r-2*r+dx, 300-dy, r);
    xy_tuple_to_string();
    ...
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(10*r-6*dx, 300, r);
    xy_tuple_to_string();
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(8*r-5*dx, 300-dy, r);
    xy_tuple_to_string();
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(8*r-5*dx, 300+dy, r);
    ...
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(10*r-12*dx, 300, r);
    xy_tuple_to_string();
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(8*r-11*dx, 300-dy, r);
    xy_tuple_to_string();
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(8*r-11*dx, 300+dy, r);
    ...
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    make_hexagon_tuple(10*r-18*dx, 300, r);
    xy_tuple_to_string();
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    // ==========================================
    //    
    make_hexagon_tuple(10*r, 300, r);
    xy_tuple_to_string();
    draw_polygon(hexagon_xy, "none", "blue", 1);
    //
    dy = 25;
    cout << "now dy = " << dy << endl;
    //
    translate_polygon(make_tuple(1.5*r, 1*dy), 
          hexagon_xy, "none", "green", 1);
    translate_polygon(make_tuple(1.5*r,-1*dy), 
          hexagon_xy, "none", "green", 1);
    //
    translate_polygon(make_tuple(4.5*r, 1*dy), 
          hexagon_xy, "none", "green", 1);
    translate_polygon(make_tuple(4.5*r,-1*dy), 
          hexagon_xy, "none", "green", 1);
    //
    translate_polygon(make_tuple(7.5*r, 1*dy), 
          hexagon_xy, "none", "green", 1);
    translate_polygon(make_tuple(7.5*r,-1*dy), 
          hexagon_xy, "none", "green", 1);
    //
    translate_polygon(make_tuple(10.5*r, 1*dy), 
          hexagon_xy, "none", "green", 1);
    translate_polygon(make_tuple(10.5*r,-1*dy), 
          hexagon_xy, "none", "green", 1);
    // ---------------------------------------
    translate_polygon(make_tuple(3.0*r, 0), 
          hexagon_xy, "none", "green", 1);
    //
    translate_polygon(make_tuple(6.0*r, 0), 
          hexagon_xy, "none", "green", 1);
    //
    translate_polygon(make_tuple(9.0*r, 0), 
          hexagon_xy, "none", "green", 1);
    //
    translate_polygon(make_tuple(12.0*r, 0), 
          hexagon_xy, "none", "green", 1);
    //
    fan_line_xy = "";
    for(int i : i_set) {
